...
namespace Leblanc
{
	...
	void IndexMesh::load(const aiMesh* mesh, const aiScene* scene)
	{
		if (mesh)
		{
			...
			unsigned int* index_ptr = new unsigned int[index_count];
			...
			memset(index_ptr, 0, sizeof(unsigned int) * index_count);
			...
			IVertexDeclaration* vertex_declaration = g_global_context.m_render_state_manager.getOrCreateVertexDeclaration(&vertex_layout_declaration);
	...
	void IndexMesh::fillIndexBuffer()
	{
		if (m_index_buffer)
		{
			unsigned int* data_segment = static_cast<unsigned int*>(m_index_buffer->lock());
			if (data_segment)
			{
				memcpy(data_segment, m_index_buffer_data, sizeof(unsigned int) * m_index_count);
				m_index_buffer->unlock();
			}
		}
	}

	void IndexMesh::setIndices(const unsigned int* indices, size_t index_count, size_t face_count)
	{
		if (index_count > m_index_count)
		{
			safe_delete_array(m_index_buffer_data);
			m_index_buffer_data = new unsigned int[index_count];
			createIndexBuffer(index_count);
		}
		m_index_count = index_count;

		if (m_index_buffer_data)
			memcpy(m_index_buffer_data, indices, index_count * sizeof(unsigned int));
		else
