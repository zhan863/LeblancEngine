...
PostProcessor::PostProcessor() : device(NULL), context(NULL)
{
}
...
void PostProcessor::Initialize(ID3D11Device* device)
{
    ...
    quadVS.Attach(CompileVSFromFile(device, L"SampleFramework11\\Shaders\\Quad.hlsl", "QuadVS", "vs_4_0", NULL, NULL, &byteCode));    
...
void PostProcessor::Render(ID3D11DeviceContext* deviceContext, ID3D11ShaderResourceView* input, ID3D11RenderTargetView* output)
{
    ...
    ID3D11SamplerState* samplerStates[2] = { pointSamplerState, linearSamplerState };
...
void PostProcessor::PostProcess(ID3D11ShaderResourceView* input, ID3D11RenderTargetView* output, ID3D11PixelShader* pixelShader, const WCHAR* name)
{
...
PostProcessor::TempRenderTarget* PostProcessor::GetTempRenderTarget(UINT width, UINT height, DXGI_FORMAT format, 
                                                                    UINT msCount, UINT msQuality)
{
    ...
    for (size_t i = 0; i < tempRenderTargets.size(); ++i)
    {
        ...
        if (!rt->InUse && rt->Width == width && rt->Height == height && rt->Format == format
                && rt->MSCount == msCount && rt->MSQuality == msQuality)
        {
            rt->InUse = true;
            return rt;
        }
...
void PostProcessor::ClearTempRenderTargetCache()
{
    for (size_t i = 0; i < tempRenderTargets.size(); ++i)
    {
